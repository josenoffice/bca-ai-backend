// BCA_AI_PRD.docx v2 — Updated with user feedback
const {
...
const WW=6240, WH=Math.round(6240*1800/2560);   // wide 2560x1800
const NW=5040, NH=Math.round(5040*1800/1600);   // narrow 1600x1800
const PW=4320, PH=Math.round(4320*1800/1600);   // pair 1600x1800
...
children.push(
  ...
  cap('Figure 1: Left — Guide Me panel with section-specific questions. Right — Guide Me panel open alongside Phase 1 results with Portfolio Mix visible'),
  sp(),
...
children.push(
  ...
  cap('Figure 2: BCA.AI home page — access registration gate'),
  sp(),
  ...
  cap('Figure 3: Guided Form — Section 1 with industry examples (left), Section with Guide Me panel and quality tips visible (right)'),
  sp(),
  ...
  cap('Figure 4: Conversational Intake — Section 1 (Strategic Alignment) with industry selector and role field'),
  sp(),
  img('42_conv_intake_step2.png', NW, NH),
  cap('Figure 5: Conversational Intake progression — previous answer shown as a chat bubble, next question presented below'),
  sp(),
  ...
  cap('Figure 6: Save and Load buttons (top right header) — available at all times during the analysis'),
  sp(),
  twoCol([
...
children.push(
  ...
  cap('Figure 7: Phase 1 results — Discovery at a Glance: 5 solutions identified, 42% budget utilisation, AI Generated method, READY for Phase 2'),
  sp(),
  img('37_phase1_portfolio_mix_solutions.png', NW, NH),
  cap('Figure 8: Solutions list with Portfolio Mix bar — showing the breakdown between Software (80%) and Process Change (20%) solution types'),
  sp(),
  img('04c_phase1_requirements_valuechain.png', WW, WH),
  cap('Figure 9: Value Chain — each solution traced left to right: Requirements it satisfies → Solution details → Benefits it delivers → Vendor recommendation'),
  sp(),
  pb()
);
...
children.push(
  ...
  cap('Figure 10: Cost Estimation at a Glance — 5 solutions priced, 42% of $680K budget utilised, within ceiling'),
  sp(),
  img('24_phase15_solution_costs.png', NW, NH),
  cap('Figure 11: Per-solution cost cards — Low/Mid/High scenario for each solution with vendor pricing range alongside'),
  sp(),
  ...
  cap('Figure 12: Phase 1 budget section — AI-suggested budget callout shown when no user budget was provided during intake'),
  sp(),
  img('52_phase15_budget_toolow.png', NW, NH),
  cap('Figure 13: Phase 1.5 — budget too low warning showing shortfall, required increase percentage, and minimum viable solution'),
  sp(),
...
children.push(
  ...
  cap('Figure 14: Left — Vendor Selection at a Glance (top fit score 94/100). Right — Vendor cards with fit score, cost range, and implementation timeline'),
  sp(),
  pb()
);
...
children.push(
  ...
  cap('Figure 15: Quality Gate — PASS WITH WARNINGS, score 72/100. Four area scores all at 100. Status: READY FOR PHASE 2'),
  sp(),
  threeCol([
  ...
  cap('Figure 16: Reflection scorecard — Phase 2 Readiness Criteria (expandable), strengths list, and Continue to Phase 2 prompt'),
  sp(),
  pb()
);
...
children.push(
  ...
  cap('Figure 17: Phase 2 Financial KPIs — Portfolio NPV $637K, ROI 356%, IRR 91%, Payback 19 months — all benchmarked against Retail industry targets'),
  sp(),
  img('06a_phase2_roi_irr_spread.png', WW, WH),
  cap('Figure 18: Plain-English KPI explanations — each metric card includes a Show calculation link so any stakeholder can verify the numbers'),
  sp(),
  pb()
);
...
children.push(
  ...
  cap('Figure 19: Phase 3 overview — Health Score 78, Linkage Score 73, 30 weeks total duration, $209K cost, 78% average confidence'),
  sp(),
  img('53_phase3_plc_cost_labels.png', NW, NH),
  cap('Figure 20: Project Timeline with PLC phase weights and approximate cost per phase — e.g. Discovery 15% (~$31K), Execution 50% (~$105K), Go-Live 10% (~$21K)'),
  sp(),
...
children.push(
  ...
  cap('Figure 21: Phase 4 Recommendation — Security & Compliance Foundation ranked #1, composite score 0.843, ROI 237%, Payback 11 months, Phase 5 Gate: PASS'),
  sp(),
  img('32_phase4_solution_ranking.png', NW, NH),
  cap('Figure 22: Full Solution Ranking table — all solutions ranked with Score, NPV, ROI, Risk, Vendor/Approach, Fit, and Payback'),
  sp(),

  h2('11.3  Pre-Report Checkpoint — Vendor Summary Card'),
  ...
  cap('Figure 23: Phase 4 vendor summary card — recommended vendor (Vanta, Fit Score 94/100) with cost range and compliance coverage before Phase 5 launch'),
  sp(),

  h2('11.4  Pre-Report Checkpoint — Value Chain Detail'),
  ...
  cap('Figure 24: Phase 4 value chain detail panel — per-solution view of Requirements → Benefits → Financial Value → Vendor/Delivery, immediately before Phase 5 launch'),
  sp(),
...
children.push(
  ...
  cap('Figure 25: Phase 5 Executive at a Glance — Portfolio PV $474K, ROI 171.1%, STRONG RETURN callout, Why This Recommendation panel'),
  sp(),
  img('34_phase5_health_dashboard.png', NW, NH),
  cap('Figure 26: Phase 5 Health Dashboard — Budget, Traceability, and Confidence RAG indicators with full Value Chain traceability map below'),
  sp(),
  p('Phase 5 outputs:'),
  ...
  cap('Figure 27: Phase 5 Alternatives Considered section — previous attempts listed with chips (e.g. ERP upgrade, custom build, SaaS trial) and a free-text explanation of why they failed'),
  sp(),
  ...
  cap('Figure 28: Phase 5 Training & Adoption Plan — conditional table covering all process change and hybrid solutions with complexity, people impact, and key activities'),
  sp(),
  ...
  cap('Figure 29: Phase 5 Recommended Next Steps — five tailored actions with vendor name, solution type, and BRD export link embedded in the text'),
  sp(),
  pb()
);
...
children.push(
  ...
  cap('Figure 30: Analyst mode — expanded navigation reveals Traceability, Ranking, Report, BRD, and BRD Standalone tabs hidden in Business mode'),
  sp(),
  twoCol([
...
children.push(
  ...
  threeCol([
    ...
    ['Phase 1 Reflection / Quality Gate', 'AI-Powered', 'Reviews linkage quality, benefit credibility, and vendor compliance coverage. Produces a structured verdict.'],
